#!/usr/bin/env python3
"""
RStudio Version Updater (No External Dependencies)
Checks for the latest stable RStudio version using only built-in Pytho    if response in ['y', 'yes']:
        if update_playbook_version(playbook_path, current_version, latest_version):
            print("🎉 Update completed successfully!")
            print(f"Updated from {current_version} to {latest_version}")braries.
"""
...
def get_latest_stable_version():
    ...
    try:
        with urllib.request.urlopen('https://posit.co/download/rstudio-desktop/', timeout=10) as response:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            if response.getcode() == 200:
                ...
                print(f"❌ Version {version} not accessible: HTTP {response.getcode()}")
                return False
    except Exception as e:
...
def main():
    ...
    print("RStudio Version Checker (Built-in Libraries)")
    print("=" * 45)
